...
class MyProblem(ea.Problem):  # 继承Problem父类
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        ones_matrix = np.ones((Vars.shape[0], 1))
        f = 0.5 * np.fliplr(np.cumprod(np.hstack([ones_matrix, Vars[:, :self.M-1]]), 1)) * np.hstack([ones_matrix, 1 - Vars[:, range(self.M - 2, -1, -1)]]) * np.tile(1 + g, (1, self.M))
