from in20201230.DimensionReductionForSparse.DE import MyProblem
...
def CCDE_Accelaration(Dim, NIND, MAX_iteration, benchmark, scale_range, groups):
...
def ProblemsOptimizationForCCDEAcceleration(Dim, NIND, MAX_iteration, benchmark, up, down, groups, elite):
    var_traces = np.zeros((MAX_iteration, Dim))
    based_population = elite
    for i in range(len(groups)):
        var_trace, obj_trace = GroupOptimizationForCCDEAcceleration(NIND, MAX_iteration, benchmark, up, down, groups[i],
                                                 based_population)
...
def GroupOptimizationForCCDEAcceleration(NIND, MAX_iteration, benchmark, up, down, group, based_population):
    problem = MyProblem.MySimpleProblemForCCDEAcceleration(group, benchmark, up, down, NIND * len(group), based_population)  # 实例化问题对象
...
def CCDEGroupOptimization(NIND, MAX_iteration, benchmark, scale_range, group, based_population):
    problem = MyProblem.MySimpleProblem(group, benchmark, scale_range, NIND * len(group), based_population)  # 实例化问题对象
