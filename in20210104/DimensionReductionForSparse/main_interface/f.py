...
def f2(Dim, func_num, NIND, m1, m2, scale_range, groups_One, groups_Lasso, method):
    ...
    best_indexes, best_obj_trace = DE.CCDE_Accelaration(Dim, NIND, m1, function, scale_range, groups_One)
    central_point = best_indexes[len(best_indexes)-1]
    delta = 0.1 * (scale_range[1] - scale_range[0])
